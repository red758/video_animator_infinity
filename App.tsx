...
const App: React.FC = () => {
  ...
  useEffect(() => {
    setIsMounted(true);
    console.log("AEON System Active");
  }, []);
    ...
    <div className="min-h-screen bg-black text-white selection:bg-indigo-600 antialiased font-sans">
      ...
      <nav className="fixed top-0 left-0 w-full z-[100] px-8 md:px-16 py-8 flex justify-between items-center pointer-events-none">
        ...
        >
          ...
          <div className="flex flex-col">
            ...
            <span className="text-[10px] font-black text-indigo-500 tracking-[0.6em] uppercase mt-2 opacity-60">Studio Core</span>
          </div>
        </div>
      ...
      {!state.url ? (
        <main className="relative min-h-screen flex flex-col items-center px-6 pt-[20vh] pb-32 overflow-hidden bg-black">
          {/* Luminous Glow Backgrounds - Placed carefully to not block content */}
          <div className="absolute top-0 left-0 w-full h-full overflow-hidden pointer-events-none z-0">
             <div className="absolute top-[-10%] left-[-10%] w-[80%] h-[80%] bg-indigo-600/10 blur-[150px] opacity-40" />
             <div className="absolute bottom-[-10%] right-[-10%] w-[80%] h-[80%] bg-purple-600/5 blur-[150px] opacity-30" />
          </div>
          
          <div className="text-center mb-24 max-w-7xl relative z-10 w-full">
            <h1 className="text-[clamp(3.5rem,10vw,9rem)] font-black tracking-[-0.04em] mb-12 leading-[0.9] italic uppercase text-white transition-opacity duration-1000">
              <span className="block opacity-90">Narrative</span>
              <span className="block text-indigo-500">Scroll</span>
            </h1>
            <p className="text-base md:text-xl text-white/40 font-light max-w-2xl mx-auto leading-relaxed tracking-[0.05em] uppercase italic border-l-4 border-indigo-600/40 pl-8 text-left">
        ...
        <section className="relative z-0">
          ...
          <section className="bg-black py-48 px-8 border-t border-white/5 relative z-10">
             <div className="max-w-7xl mx-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                   {[
                     ...
                     <div key={i} className="p-10 bg-white/[0.02] border border-white/5 rounded-[2.5rem] hover:bg-white/[0.04] transition-colors">
                        <div className="mb-8 w-16 h-16 bg-black border border-white/10 flex items-center justify-center text-indigo-500 rounded-2xl">{step.icon}</div>
                        <h3 className="text-xl font-black italic uppercase tracking-widest mb-3">{step.title}</h3>
                        <p className="text-white/30 text-[13px] font-medium uppercase tracking-wider leading-relaxed">{step.desc}</p>
                     </div>
                   ))}
                </div>
             </div>
          ...
          <footer className="relative min-h-[60vh] flex flex-col items-center justify-center bg-black border-t border-white/5 z-10">
            <div className="text-center z-10 px-8">
              <h2 className="text-[clamp(2rem,6vw,5rem)] font-black tracking-[-0.02em] mb-10 italic uppercase text-white/90">Experience Concluded.</h2>
              <div className="flex flex-col md:flex-row justify-center gap-6">
                ...
                <button onClick={handleReset} className="px-16 py-6 border-2 border-white/10 text-white/40 font-black text-[12px] uppercase tracking-[0.2em] rounded-full hover:bg-white/5 hover:text-white transition-all">
                  New Narrative
                </button>
              </div>
            </div>
            
            <div className="absolute bottom-12 flex justify-between w-full px-12 text-[10px] font-black tracking-[1em] text-white/5 uppercase italic">
      ...
      <div className="fixed inset-0 pointer-events-none z-[9999] opacity-[0.02] bg-[url('https://grainy-gradients.vercel.app/noise.svg')] mix-blend-overlay" />
    </div>
  );
};
